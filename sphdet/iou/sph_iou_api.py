...
from .kld_iou import kld_kent_iou
import numpy as np
...
def bfov_to_kent(annotations, epsilon=1e-6):
    ...
    varphi = (torch.deg2rad(data_fov_v) ** 2) / 12 + epsilon  # Horizontal variance
    vartheta = torch.sin(alpha)*(torch.deg2rad(data_fov_h) ** 2) / 12 + epsilon  # Vertical variance
    ...
    '''else:
        ...
        kent_dist = torch.stack([alpha, eta, kappa, beta], dim=1)
...
def kld_iou(bboxes1, bboxes2, mode = 'iou', is_aligned = False):
    #from sphdet.losses.kld_loss import kld_default
    assert mode in ['iou']
    ...
    bboxes1 = transform(bboxes1)
    bboxes2 = transform(bboxes2)

    #bboxes1, bboxes2 = jiter_spherical_bboxes(bboxes1, bboxes2)
    overlaps = kld_kent_iou(bboxes1, bboxes2)
