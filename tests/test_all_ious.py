...
import torch
import numpy as np
from sphdet.iou import unbiased_iou, sph2pob_standard_iou, sph2pob_legacy_iou, sph_iou, fov_iou, naive_iou, kld_iou
...
def _get_infos_about_mutil_methods(size, theta_range=(0, 360), phi_range=(0, 180), alpha_range=(0, 360), beta_range=(0, 360), mode='both', device='cpu'):
    ...
    with torch.no_grad():
        ...
        kld_info = _get_info_about_single_method(data=(pt, gt), iou_calculator=kld_iou, device=device, mode=mode, gold_iou=gold_iou)
